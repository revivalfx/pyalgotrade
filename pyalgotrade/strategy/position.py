...
class ForexPosition(Position):
    ...
    def getLotSize(self):
        """Returns the lotSize for a Forex Postion.
        This will be a possitive number for a long position, and a negative 
        number for a short position.

        .. note::
            If the entry order was not filled, or if the position is closed, 
            then the number of shares will be 0.
        """
    ...
    def exitTakeProfit(self, takeProfitPrice, goodTillCanceled=None):
        """Submits a stop order to close this position.
        ...
        self.__state.exit(self, takeProfitPrice, None, goodTillCanceled)
    ...
    def onOrderEvent(self, orderEvent):
        ...
        if orderEvent.getEventType() in (broker.OrderEvent.Type.PARTIALLY_FILLED, broker.OrderEvent.Type.FILLED):
            ...
            # roundQuantity is used to prevent bugs like the one triggered in testcases.bitstamp_test:TestCase.testRoundingBug
            if order.isBuy():
                self.__lotSize = order.getInstrumentTraits().roundQuantity(self.__lotSize + execInfo.getQuantity())
                #self.__shares = order.getInstrumentTraits().roundQuantity(self.__shares + execInfo.getQuantity())
            else:
                self.__lotSize = order.getInstrumentTraits().roundQuantity(self.__lotSize - execInfo.getQuantity())
                #self.__shares = order.getInstrumentTraits().roundQuantity(self.__shares - execInfo.getQuantity())
