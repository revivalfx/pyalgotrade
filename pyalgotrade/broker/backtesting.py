...
class Broker(broker.Broker):
    ...
    def commitOrderExecution(self, order, dateTime, fillInfo):
        ...
        if resultingCash >= 0 or self.__allowNegativeCash:
            ...
            self.__logger.debug("Broker::commitOrderExecution: Not enough cash to fill %s order [%s] for %s share/s" % (
                order.getInstrument(),
                order.getId(),
                order.getRemaining()
            ))
...
class ForexBroker(Broker):
    """Backtesting ForexBroker.
    ...
    def getLotSize(self, instrument):
        return self.__lotSize.get(instrument, 0)

    def getShares(self, instrument):
        #return self.getLotSize(instrument)
        raise Exception
    ...
    def getActiveInstruments(self):
        #return [instrument for instrument, shares in self.__shares.iteritems() if shares != 0]
        return [instrument for instrument, lots in self.__lotSize.iteritems() if lots != 0]

    def __getEquityWithBars(self, bars):
        ret = self.getCash()
        if bars is not None:
            #for instrument, shares in self.__shares.iteritems():
            for instrument, lots in self.__lotSize.iteritems():
                instrumentPrice = self._getBar(bars, instrument).getClose(self.getUseAdjustedValues())
                ret += instrumentPrice * lots
        return ret
    ...
    def commitOrderExecution(self, order, dateTime, fillInfo):
        ...
        if resultingCash >= 0 or self.__allowNegativeCash:
            ...
            # Commit the order execution.
            self.__cash = resultingCash
            updatedPosition = order.getInstrumentTraits().roundQuantity(
                self.getLotSize(order.getInstrument()) + sharesDelta
            )
            if updatedPosition == 0:
                #del self.__shares[order.getInstrument()]
                del self.__lotSize[order.getInstrument()]
            else:
                #self.__shares[order.getInstrument()] = updatedShares
                self.__lotSize[order.getInstrument()] = updatedPosition
            ...
            self.__logger.debug("ForexBroker::commitOrderExecution: Not enough cash to fill %s order [%s] for %s share/s" % (
                order.getInstrument(),
                order.getId(),
                order.getRemaining()
            ))
